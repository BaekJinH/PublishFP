document.addEventListener('DOMContentLoaded', () => {
  ...
  const myBlossomSceneConfig = {
    id: "end-blossom",
    numPetals: 50, // contact 섹션에서 더 많은 꽃잎
    petalsTypes
  };
  ...
  const observerCallback = (entries, observer) => {
    entries.forEach(entry => {
      if (entry.isIntersecting) {
        setTimeout(() => {
          myBlossomScene.container.style.opacity = '0'; 
          setTimeout(() => {
            specialContent.classList.add('screen-active'); 
          }, 500);
          setTimeout(() => {
            contactList.classList.add('list-active');
          }, 4000);
        }, 1500);
        // observer.unobserve(entry.target);
      } else {
        myBlossomScene.container.style.opacity = '1';
        specialContent.classList.remove('screen-active');
        contactList.classList.remove('list-active');
      }
    });
  };
